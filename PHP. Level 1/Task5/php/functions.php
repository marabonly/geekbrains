...
{
    $sql = "INSERT INTO shop(name, path, size, views) VALUES('" . $filename . "','" . $targetFileBig . "'," . $size . ",0)";
...
{
    $sql = "SELECT id, path, views FROM shop ORDER BY views DESC";
...
{
    $sql = "SELECT path, views FROM shop WHERE id = " . $id;
...
{
    $sql = "UPDATE shop SET views = " . $newViews . " WHERE id = " . $id;
